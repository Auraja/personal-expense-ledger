...
async function seed(){
  ...
  for(const [name,type] of [['Main','ewallet'],['Tabungan Bank Jago (GoPay)','bank'],['BRI','bank'],['blu by BCA','bank']]) await db.account.upsert({where:{name},create:{name,type,initialBalance:0},update:{}});
